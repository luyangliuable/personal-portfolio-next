...
class NavBar extends Component<INavbarProps, INavbarState> {
    ...
    renderDropdownMenu = (links: ILink[] | undefined): void => {
        if (links !== undefined) {
            this.showDropdownMenu();
            this.setState({ dropdownMenuLinkDisplay: links.map((item, _) => this.renderLink(item)) });
    ...
    renderLink = (link: ILink, isSubLink: boolean = true) => {
        ...
        return (
            <Link
                ...
                key={link.name}
                href={targetPath ?? "/"}
                className="navbar-item flex justify-center items-center">
                {navLinkContent}
            </Link>
        );
    }
                ...
                <article
                    ...
                    <section className="navbar-content flex items-center">
                        <div className="logo__wrapper">
                            <Link href="/"><h1 className="logo">{name}</h1></Link>
                        </div>
                        <nav ref={this.navbarLeft} className="navbar-left flex flex-row">
                            {links.map((item, _) => this.renderLink(item, false))}
