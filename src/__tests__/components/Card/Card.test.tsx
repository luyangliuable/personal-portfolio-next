...
vi.mock("@/components/Image/Image", () => ({ default: ({ src, alt }: any) => <img src={src} alt={alt} /> }));
vi.mock("@/components/TagCloud/TagCloud", () => ({ default: ({ tags }: any) => <div>{tags?.join(",")}</div> }));
vi.mock("@/components/Utility/MouseUtility", () => ({ cardGradientEffect: vi.fn() }));

describe("Card", () => {
    const baseProps = { link: "/post", image: "/image.png", authorImage: "/author.png", author: "Author", heading: "Heading", minuteRead: 5, tags: ["ts"], date_created: "2024-01-02T00:00:00Z" };
    ...
    it("extracts only internal llcode.tech paths from absolute URLs.", () => {
        ...
        expect(instance.extractRouteFromURL("https://llcode.tech/blog/a")).toBe("/blog/a");
        expect(instance.extractRouteFromURL("https://example.com/blog/a")).toBeNull();
    });
});
