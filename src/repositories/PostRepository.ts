...
class PostRepository extends Repository {
    ...
    static readonly BASE_URL: string = "https://llcode.tech/api/posts";
    ...
    async getFeaturedPostList(): Promise<BlogPostResponse[]> {
        ...
        const featuredPosts = postList.filter(
            (post: BlogPostResponse) => post.is_featured,
        );
    ...
    async getRelatedPosts(
        ...
        try {
            ...
            const postsWithCommonTags = allPosts.map(
                (post: BlogPostResponse) => ({
                    post,
                    commonTags: post.tags.filter((tag: string) =>
                        tags.includes(tag),
                    ).length,
                }),
            );
            postsWithCommonTags.sort(
                (a: any, b: any) => b.commonTags - a.commonTags,
            );
            return postsWithCommonTags
                .slice(0, numberOfResults + 1)
                .map((item: any) => item.post)
                .filter(
                    (post: BlogPostResponse) => post._id.$oid !== currentPostId,
                );
        } catch (error) {
