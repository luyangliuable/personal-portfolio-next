...
const ExperienceSection: React.FC<IExperienceSectionProps> = ({ scrolled }) => {
  ...
  useEffect(() => {
    ...
    if (state.isLocked && state.lockPosition !== undefined) {
      scrolled = scrolled ?? 0;
      const scrollAmount =
        state.lockPosition - proximityYToLockPosition - scrolled;
  ...
  const lockPosition = (): void => {
    if (scrolled !== undefined) {
      const isNotPastUnlockPosition =
        state.unlockPosition === null || scrolled < state.unlockPosition;
  ...
  const isBeforeLockPosition = (
    ...
    const isBeforeLockPosition =
      ...
      scrolled + proximityYToLockPosition < lockPosition;
