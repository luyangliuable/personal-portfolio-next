    ...
    const sketch = (p: p5) => {
          ...
          for (let x = 0; x < cols; x++) {
            ...
            if (y > maxY - 13) {
              terrain[x][y] = p.map(
                ...
                heightVal * heightScale * distScale * gullyEffect,
              );
      ...
      function getMagmaColor(normalizedHeight: number) {
        ...
        if (normalizedHeight < 0.33)
          return p.lerpColor(c2, c3, (normalizedHeight - 0.16) * 5.88);
        if (normalizedHeight < 0.5)
          return p.lerpColor(c3, c4, (normalizedHeight - 0.33) * 5.88);
        if (normalizedHeight < 0.66)
          return p.lerpColor(c4, c5, (normalizedHeight - 0.5) * 6.25);
        if (normalizedHeight < 0.83)
          return p.lerpColor(c5, c6, (normalizedHeight - 0.66) * 5.88);
