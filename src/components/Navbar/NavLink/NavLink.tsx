...
const NavLink: React.FC<INavLinkProps> = ({
  ...
  const navLinkContent = useMemo(
    () =>
      [link.name, link.sublinks && <AiOutlineDown key="down-icon" />].filter(
        Boolean,
      ),
    [],
  );
  ...
  return React.createElement(
    ...
    React.createElement(
      "div",
      {
        className: "flex flex-row justify-start items-center select-none",
      },
      link.emoji &&
    ...
    React.createElement(
      "div",
      {
        className: cl("blur-boundary flex justify-center items-center mx-1", {
