...
class FeaturedContentSection extends Component<IFeaturedContentSectionProps, IFeaturedContentSectionState> {
  ...
  private featuredSectionRef: RefObject<HTMLDivElement>;
  ...
  constructor(props: IFeaturedContentSectionProps) {
    ...
    this.featuredSectionRef = createRef<HTMLDivElement>(); // Add this line
  ...
  showAllElements = () => {
    ...
    if (this.featuredSectionRef.current) this.featuredSectionRef.current.classList.remove('featured-section-with-before');
  }
    ...
    return (
      <LandingPageCard className="mb-20" heading="Featured Content" landingPageCardType="fitContent" blendWithBackground={true}>
        <section ref={this.currentComponentRef} className="flex flex-col items-center">
          <div ref={this.featuredSectionRef} className="featured-section featured-section-with-before w-full flex flex-row justify-center items-start">
            <GalleryItem
