...
const NavBarMainSection: React.FC<INavbarProps> = ({ links }) => {
  ...
  const handleScroll = useCallback(() => {
    ...
    if (st > lastScrollTopRef.current && !isNavbarHidden && navBarDetached) {
  ...
  const setNavBarHidden = useCallback(
    (set: boolean) => {
      ...
      if (navbar.current && navbar.current.parentElement) {
        ...
        toggleClassName(navbar.current.parentElement, set, "navbar--hidden");
        setState((prev) => {
          return {
            ...prev,
            isNavbarHidden: set,
          ...
          <nav ref={navbarLeft} className="navbar-left flex flex-row">
            ...
            >
              <div
                ...
                style={{
                  width:
                    state.dropdownMenuLinkDisplay.length > 5 ? "900px" : "auto",
                }}
